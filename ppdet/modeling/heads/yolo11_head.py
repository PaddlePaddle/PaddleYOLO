...
from ppdet.modeling.backbones.yolo11_csp_darknet import Conv, DWConv
from ppdet.modeling.layers import MultiClassNMS

__all__ = ["YOLO11Head", "YOLO11InsHead"]
...
class DFL(nn.Layer):
    ...
    def __init__(self, c1=16):
        ...
        super().__init__()
        self.conv = nn.Conv2D(c1, 1, 1, bias_attr=False)
        self.conv.weight.stop_gradient = True
        x = paddle.arange(c1, dtype="float32")
        self.conv.weight.set_value(x.reshape((1, c1, 1, 1)))
        self.c1 = c1

    def forward(self, x):
        """Applies a transformer layer on input tensor 'x' and returns a tensor."""
        b, _, a = x.shape  # batch, channels, anchors
        x_view = x.reshape((b, 4, self.c1, a))
        x_trans = paddle.transpose(x_view, perm=[0, 2, 1, 3])
        x_softmax = F.softmax(x_trans, axis=1)
        return self.conv(x_softmax).reshape((b, 4, a))


@register
class YOLO11Head(nn.Layer):
    __shared__ = [
        "num_classes",
        "eval_size",
        "act",
    ...
    def __init__(
        ...
        super(YOLO11Head, self).__init__()
        ...
        # Initialize detection head layers - regression branch
        self.cv2 = nn.LayerList()
        for in_c in self.in_channels:
            self.cv2.append(
                nn.Sequential(
                    *[
                        Conv(in_c, c2, 3, 1, act=get_activation(act)),
                        Conv(c2, c2, 3, 1, act=get_activation(act)),
                        nn.Conv2D(
                            c2,
                            self.reg_max * 4,
                            1,
        ...
        # Initialize detection head layers - classification branch
        ...
        for in_c in self.in_channels:
            self.cv3.append(
                nn.Sequential(
                    *[
                        nn.Sequential(
                            DWConv(in_c, in_c, 3, 1, act=get_activation(act)),
                            Conv(in_c, c3, 1, 1, act=get_activation(act)),
                        ),
                        nn.Sequential(
                            DWConv(c3, c3, 3, 1, act=get_activation(act)),
                            Conv(c3, c3, 1, 1, act=get_activation(act)),
                        ),
                        nn.Conv2D(
                            c3,
                            self.num_classes,
    ...
    def _initialize_biases(self):
        for a, b, s in zip(self.cv2, self.cv3, self.fpn_strides):
    ...
    def forward_train(self, feats, targets):
        # Generate anchors for grid cells
        ...
        # Process feature maps through detection heads
        outputs = []
        for i, feat in enumerate(feats):
            # Concatenate regression and classification outputs
            output = paddle.concat([self.cv2[i](feat), self.cv3[i](feat)], axis=1)
            outputs.append(output)

        # Calculate loss
        return self.get_loss(
            outputs, [anchors, anchor_points, num_anchors_list, stride_tensor], targets
        )
    ...
    def forward_eval(self, feats):
        ...
        return self._inference(outputs, anchor_points, stride_tensor)

    def _inference(self, outputs, anchor_points, stride_tensor):
        # Get shape for dynamic anchor generation
        shape = outputs[0].shape  # BCHW

        # Concatenate outputs from different feature levels
        bs = shape[0]
        ...
        cls_preds = outputs_cat[:, self.reg_max * 4 :, :]

        # Apply DFL to box predictions
        box_preds = self.dfl(box_preds)

        # Convert distances to bounding boxes
        pred_bboxes = batch_distance2bbox(anchor_points, box_preds.transpose([0, 2, 1]))
        pred_bboxes = pred_bboxes * stride_tensor

        # Apply sigmoid to class predictions
        pred_scores = F.sigmoid(cls_preds.transpose([0, 2, 1]))

        return pred_scores, pred_bboxes, anchor_points, stride_tensor

    def _generate_anchors(self, feats=None, dtype="float32"):
        # Generate anchor points for each feature level
        anchor_points = []
        stride_tensor = []
        for i, stride in enumerate(self.fpn_strides):
            ...
            # Stack coordinates and create stride tensor
            anchor_point = paddle.cast(
                paddle.stack([shift_x, shift_y], axis=-1), dtype=dtype
            )
            anchor_points.append(anchor_point.reshape([-1, 2]))
            stride_tensor.append(paddle.full([h * w, 1], stride, dtype=dtype))

        # Concatenate all anchor points and strides
        anchor_points = paddle.concat(anchor_points)
        stride_tensor = paddle.concat(stride_tensor)
        return anchor_points, stride_tensor

    def _bbox2distance(self, points, bbox, reg_max=15, eps=0.01):
        # Convert bounding box coordinates to distances
        x1y1, x2y2 = paddle.split(bbox, 2, -1)
        lt = points - x1y1  # left-top
        rb = x2y2 - points  # right-bottom
        return paddle.concat([lt, rb], -1).clip(0, reg_max - eps)

    def _df_loss(self, pred_dist, target, lower_bound=0):
        # Distribution focal loss calculation
        target_left = paddle.cast(target.floor(), "int64")
        target_right = target_left + 1
        weight_left = target_right.astype("float32") - target
        weight_right = 1 - weight_left

        # Weighted cross-entropy for left and right targets
        loss_left = (
            F.cross_entropy(pred_dist, target_left - lower_bound, reduction="none")
            * weight_left
        )
        loss_right = (
            F.cross_entropy(pred_dist, target_right - lower_bound, reduction="none")
            * weight_right
        )

        return (loss_left + loss_right).mean(-1, keepdim=True)

    def get_loss(self, outputs, anchor_info, gt_meta):
        # Unpack anchor information
        anchors, anchor_points, num_anchors_list, stride_tensor = anchor_info

        # Extract box and class predictions from outputs
        bs = outputs[0].shape[0]

        # Process outputs to extract predictions
        cls_preds_list = []
        reg_preds_list = []
        reg_dist_preds_list = []
        ...
        for i, output in enumerate(outputs):
            ...
            # Split output into box and class predictions
            reg_dist_pred = output[:, : self.reg_max * 4, :, :]
            cls_pred = output[:, self.reg_max * 4 :, :, :]

            # Process regression predictions
            reg_dist_pred = reg_dist_pred.reshape(
                [bs, 4, self.reg_max, h * w]
            ).transpose([0, 3, 1, 2])  # [bs, h*w, 4, reg_max]
            reg_pred = (
                F.softmax(reg_dist_pred, axis=3)
                .matmul(paddle.arange(self.reg_max).astype("float32").reshape([-1, 1]))
                .squeeze(-1)
            )  # [bs, h*w, 4]

            # Store predictions
            cls_preds_list.append(cls_pred)
            reg_preds_list.append(reg_pred.transpose([0, 2, 1]).reshape([bs, 4, h, w]))
            reg_dist_preds_list.append(reg_dist_pred)

        # Reshape and concatenate predictions
        flatten_cls_preds = [
            cls_pred.transpose([0, 2, 3, 1]).reshape([bs, -1, self.num_classes])
            for cls_pred in cls_preds_list
        ]
        flatten_pred_bboxes = [
            bbox_pred.transpose([0, 2, 3, 1]).reshape([bs, -1, 4])
            for bbox_pred in reg_preds_list
        ]
        flatten_pred_dists = [
            bbox_pred_org.reshape([bs, -1, self.reg_max * 4])
            for bbox_pred_org in reg_dist_preds_list
        ]

        flatten_dist_preds = paddle.concat(
            flatten_pred_dists, 1
        )  # [bs, total_anchors, reg_max*4]
        pred_scores = paddle.concat(
            flatten_cls_preds, 1
        )  # [bs, total_anchors, num_classes]
        pred_distri = paddle.concat(flatten_pred_bboxes, 1)  # [bs, total_anchors, 4]

        # Convert distances to bounding boxes
        anchor_points_s = anchor_points / stride_tensor
        pred_bboxes = batch_distance2bbox(
            anchor_points_s, pred_distri
        )  # xyxy [bs, total_anchors, 4]
        pred_bboxes = pred_bboxes * stride_tensor  # scale by stride

        # Get ground truth data
        gt_labels = gt_meta["gt_class"]  # [bs, max_gt, 1]
        gt_bboxes = gt_meta["gt_bbox"]  # xyxy max=640 # [bs, max_gt, 4]
        ...
        # Classification loss
        loss_cls = self.bce(
            pred_scores, assigned_scores
        ).sum()  # [bs, total_anchors, num_classes]

        # Normalize by sum of assigned scores
        assigned_scores_sum = assigned_scores.sum()
        if paddle.distributed.get_world_size() > 1:
            paddle.distributed.all_reduce(assigned_scores_sum)
            assigned_scores_sum /= paddle.distributed.get_world_size()
        assigned_scores_sum = paddle.clip(assigned_scores_sum, min=1.0)
        ...
        # Calculate losses for positive samples
        if num_pos > 0:
            # IoU (CIoU) loss for bounding boxes
            bbox_mask = mask_positive.unsqueeze(-1).tile([1, 1, 4])
            pred_bboxes_pos = paddle.masked_select(pred_bboxes, bbox_mask).reshape(
                [-1, 4]
            )
            ...
            # Optional L1 loss for monitoring convergence
            ...
            dist_mask = mask_positive.unsqueeze(-1).tile([1, 1, self.reg_max * 4])
            pred_dist_pos = paddle.masked_select(flatten_dist_preds, dist_mask).reshape(
                [-1, 4, self.reg_max]
            )

            # Convert target boxes to distribution targets
            assigned_ltrb = self._bbox2distance(
                anchor_points_s, assigned_bboxes, reg_max=self.reg_max - 1, eps=0.01
            )
            assigned_ltrb_pos = paddle.masked_select(assigned_ltrb, bbox_mask).reshape(
                [-1, 4]
            )
            ...
            # Zero losses if no positive samples
            loss_iou = flatten_dist_preds.sum() * 0.0
            loss_dfl = flatten_dist_preds.sum() * 0.0
            loss_l1 = flatten_dist_preds.sum() * 0.0
        ...
        if self.print_l1_loss:
            # Add L1 loss for convergence monitoring
            out_dict.update({"loss_l1": loss_l1 * total_bs})
        return out_dict

    def post_process(self, head_outs, im_shape, scale_factor, infer_shape=[640, 640]):
        # Unpack head outputs
        pred_scores, pred_bboxes, anchor_points, stride_tensor = head_outs

        # Handle post-processing exclusion for benchmarking
        if self.exclude_post_process:
            ...
            # Handle NMS exclusion for benchmarking
            if self.exclude_nms:
...
class MaskProto(nn.Layer):
    # YOLO11 mask Proto module for instance segmentation models
    def __init__(self, ch_in, num_protos=256, num_masks=32, act="silu"):
        super().__init__()
        self.conv1 = Conv(ch_in, num_protos, 3, 1, act=get_activation(act))
        self.upsample = nn.Conv2DTranspose(
            num_protos, num_protos, 2, 2, 0, bias_attr=True
        )
        self.conv2 = Conv(num_protos, num_protos, 3, 1, act=get_activation(act))
        self.conv3 = Conv(num_protos, num_masks, 1, 1, act=get_activation(act))
...
class YOLO11InsHead(nn.Layer):
    ...
    def __init__(
        ...
        super(YOLO11InsHead, self).__init__()
        assert len(in_channels) > 0, "len(in_channels) should > 0"
        ...
        for in_c in self.in_channels:
            ...
            self.conv_cls.append(
                nn.Sequential(
                    *[
                        Conv(in_c, c3, 3, 1, act=get_activation(act)),
                        Conv(c3, c3, 3, 1, act=get_activation(act)),
                        nn.Conv2D(
                            c3,
                            self.num_classes,
                            1,
            ...
            self.conv_ins.append(
                nn.Sequential(
                    *[
                        Conv(in_c, c4, 3, 1, act=get_activation(act)),
                        Conv(c4, c4, 3, 1, act=get_activation(act)),
                        nn.Conv2D(
                            c4,
                            self.num_masks,
                            1,
        ...
        self.proto = MaskProto(
            in_channels[0], self.num_protos, self.num_masks, act=get_activation(act)
        )
    ...
    def forward_train(self, feats, targets):
        ...
        for i, feat in enumerate(feats):
            ...
            bbox_preds = (
                F.softmax(bbox_dist_preds, axis=3)
                .matmul(self.proj.reshape([-1, 1]))
                .squeeze(-1)
            )
    ...
    def forward_eval(self, feats):
        ...
        mask_coeff_list = [
            mask_coeff.transpose([0, 2, 3, 1]).reshape([-1, size, self.num_masks])
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        imgsz = (
            paddle.to_tensor(cls_scores[0].shape[2:]) * self.fpn_strides[0]
        )  # image size (h,w)
        ...
        flatten_mask_coeff_preds = [
            mask_coeff.transpose([0, 2, 3, 1]).reshape([bs, -1, self.num_masks])
            for mask_coeff in pred_mask_coeffs
        ]
        ...
        if tuple(gt_segms.shape[-2:]) != (mask_h, mask_w):  # downsample
            gt_segms = F.interpolate(
                gt_segms, (mask_h, mask_w), mode="nearest"
            ).reshape([bs, -1, mask_h * mask_w])
        ...
        assigned_masks = paddle.gather(
            gt_segms.reshape([-1, mask_h * mask_w]), assigned_gt_index.flatten(), axis=0
        )
        ...
        if num_pos > 0:
            ...
            loss_iou = flatten_dist_preds.sum() * 0.0
            loss_dfl = flatten_dist_preds.sum() * 0.0
            loss_mask = flatten_dist_preds.sum() * 0.0
            loss_l1 = flatten_dist_preds.sum() * 0.0
    ...
    def calculate_segmentation_loss(
        ...
        marea = xyxy2xywh(target_bboxes_normalized)[..., 2:].prod(2).unsqueeze(-1)
    ...
    def single_mask_loss(gt_mask, pred, proto, xyxy, area):
        ...
        return (crop_mask(loss, xyxy).mean(axis=(1, 2)) / area.squeeze(-1)).sum()

    def post_process(
        self, head_outs, im_shape, scale_factor, infer_shape=[640, 640], rescale=True
    ):
        ...
        if self.with_mask and bbox_num.sum() > 0:
            ...
            mask_logits = process_mask_upsample(
                mask_feat[0], mask_coeffs, bbox_pred[:, 2:6], infer_shape
            )
            if rescale:
...
def xyxy2xywh(x):
    ...
    y = (
        paddle.empty_like(x) if isinstance(x, paddle.Tensor) else np.empty_like(x)
