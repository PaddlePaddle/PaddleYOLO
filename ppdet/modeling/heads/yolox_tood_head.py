...
class TaskDecomposition(nn.Layer):
    ...
    def forward(self, feat, avg_feat):
        b, _, h, w = get_static_shape(feat)#feat:[4, 1536, 17, 19],
        weight = F.relu(self.la_conv1(avg_feat))#[4, 32, 1, 1]
        weight = F.sigmoid(self.la_conv2(weight)).unsqueeze(-1)#[4, 6, 1, 1, 1]
        feat = paddle.reshape(
            feat, [b, self.stacked_convs, self.feat_channels, h, w]) * weight#[4, 6, 256, 17, 19]
        feat = self.reduction_conv(feat.flatten(1, 2))#[4,256, 17, 19]
        feat = F.relu(feat)#[4, 256, 136, 152]
        return feat#将FPN每层经过TAP模块生成的特征层融合成一个NCHW维度的特征层。
...
class TOODHead(nn.Layer):
    ...
    def __init__(self,
        ...
        self.cls_decomp = nn.LayerList()
        self.reg_decomp = nn.LayerList()
        self.cls_decomp.append(
                TaskDecomposition(self.feat_channels, self.stacked_convs,#有问题继续改！！！！！！！！
    ...
    def forward(self, feats):
        ...
        for k,(feat, scale_reg, anchor_centers, stride) in enumerate( zip(
            ...
            for inter_conv in self.inter_convs:
                ...
                inter_feats.append(feat)          #TAP有6个特征层
            feat = paddle.concat(inter_feats, axis=1)#len(inter_feats)=6，输入输出尺度HW不变

            # task decomposition
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))#avg_feat.shape:[4, 1536, 1, 1]
            cls_feat = self.cls_decomp[k](feat, avg_feat)#[4, 256, H, W]
            reg_feat = self.reg_decomp[k](feat, avg_feat)

            # cls prediction and alignment
            cls_logits = self.tood_cls[k](cls_feat)#[4, 80, H, W]
            if self.use_align_head:
                cls_prob = F.relu(self.cls_prob_conv1[k](feat))# channel*24
                cls_prob = F.sigmoid(self.cls_prob_conv2[k](cls_prob))#channel=1
        ...
        return cls_score_list, bbox_pred_list, anchors, num_anchors_list, stride_tensor#没有采用节藕头
