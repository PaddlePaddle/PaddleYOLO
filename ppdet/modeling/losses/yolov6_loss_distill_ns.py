...
class loss_distill_ns:
    ...
    def __init__(
            ...
            print_l1_loss=True):
        super(loss_distill_ns, self).__init__()
    ...
    def __call__(self, head_outs, t_outputs, epoch_num, max_epoch, temperature):
        feats, pred_scores, pred_distri, pred_ltbrs, gt_meta = head_outs
        ...
        pred_bboxes_lrtb = batch_distance2bbox(anchor_points_s, pred_ltbrs)  # iou branch
...
class BboxLoss(nn.Layer):
    ...
    def forward(self, pred_dist, pred_bboxes_lrtb, pred_bboxes, t_pred_dist, t_pred_bboxes, temperature, anchor_points,
        ...
        if num_pos > 0:
            ...
            pred_bboxes_lrtb_pos = paddle.masked_select(pred_bboxes_lrtb,
                                                        bbox_mask).reshape([-1, 4])
        ...
        return (loss_iou + loss_iou_lrtb), loss_dfl, d_loss_dfl, loss_l1
